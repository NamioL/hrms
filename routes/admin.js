...
/**
 * Description:
 * Displays monthly overview of employees
 *
 * Known Bugs: None
 */
 router.get('/monthly-overview/', function viewHome(req, res, next) {
    ...
    const userLeavs = userId => {
        return new Promise( (resolve, reject) => {
            Leave.find({applicantID: userId}, function(err, data) {
                err ? reject(err) : resolve(data)
            })
        })
    }
    ...
    const userAttendance = userId => {
        return new Promise( (resolve, reject) => {
            Attendance.find({employeeID: userId}, function(err, data) {
                err ? reject(err) : resolve(data)
            })
        })
    }
    ...
    const callUsers = async () => {
        ...
        for(let i=0; i<result.length; i++) {
            ...
            const leave = await (userLeavs(result[i]._id));
            const attendance = await( userAttendance(result[i]._id));

            resultUsers.push({
                name: result[i].name,
                leaves: leave.length,
                attendances: attendance.length,
                salary: result[i].hourlySalary ? attendance.length * 8 * result[i].hourlySalary : 0,
    ...
    callUsers().then( function( users ){ 
        res.render('Admin/montlyOverview', {
        title: 'Admin Home',
        csrfToken: req.csrfToken(),
        userName: req.session.user.name,
        users: users,
...
router.get('/add-employee', function addEmployee(req, res, next) {
    ...
    User.find({ Type: { $ne :'employee'} }, function( err, data ) {
        if(err) {
            console.log(err)
        } else {
...
router.get('/employee-profile/:id', function getEmployeeProfile(req, res, next) {
    ...
    User.findById(employeeId, function getUser(err, user) {
        ...
        User.findById( user.manager, function( err, manager) {
...
router.get('/edit-employee/:id', function editEmployee(req, res, next) {
    ...
    User.findById(employeeId, function getUser(err, user) {
        ...
        User.find({ Type: { $ne :'employee'} }, function( err, managers ) {
...
router.post('/edit-employee/:id', function editEmployee(req, res) {
    ...
    User.findById(employeeId, function getUser(err, user) {
        ...
        user.hourlySalary = req.body.salary;
...
/**
 * Description:
 * Gets id of the user from the request body.
 * Gets current date.
 * Marks the attendance of that user in Attendance Schema.
 *
 *
 ...
 */
 ...
 router.post('/mark-user-attendance', function markAttendance(req, res, next) {
    ...
    Attendance.find({
        ...
        if (docs.length > 0) {
            ...
            var newAttendance = new Attendance();
            newAttendance.employeeID = req.body.user_id;
            newAttendance.year = new Date().getFullYear();
            newAttendance.month = new Date().getMonth() + 1;
            newAttendance.date = new Date().getDate();
            newAttendance.present = 1;
